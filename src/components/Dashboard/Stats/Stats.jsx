...
const Stats = ({ levels }) => {
    ...
    useEffect(() => {
        if (user !== null && user.id !== '0') {
            apiTransactions.getUserTransactions(skip, limit, user?.id)
                .then(({ data }) => {
    ...
    const handleLoadMore = () => {
        setSkip(skip + 10)
        apiTransactions.getUserTransactions(skip + 10, limit, user?.id)
            ...
            <Table responsive>
                ...
                <tbody>
                    {transactions !== null && transactions.map((transaction, index) => {
                        ...
                        const slicedAddressWallet = transaction.wallet?.substring(0, 5)
                            + "..."
                        ...
                        return <tr key={index}>
                            ...
                            <td>
                                {slicedAddressWallet}
                                <CopyToClipboard text={transaction.wallet} onCopy={() => toast.success("Address copied", { autoClose: 1000 })}>
