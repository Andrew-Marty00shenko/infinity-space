...
const Levels = () => {
  ...
  const handleClickBuyLevel = async (level, levelId) => {
    if (!viewerMode) {
      ...
      if (levels[levelId - 2]?.status === false) {
        ...
        if (wallet.toLowerCase() === user?.wallet.toLowerCase()) {
          busdContract.methods
            ...
            .then((res) => {
              contract.methods["buyLevel(uint256)"](Number(levelId))
                .send()
                ...
                .then((res) => {
                  toast.success(`${t("global:level-success")} ${levelId}`, {
                    autoClose: 1000,
                  });
                  dispatch(getUserData(wallet));
                })
                .on("error", (error) => {
                  toast.error(t("global:err"));
                });
            });
        } else {
          busdContract.methods
            ...
            .then((res) => {
              contract.methods["buyLevel(uint256,uint256)"](
                Number(levelId),
                Number(localStorage.getItem("uplineId"))
              )
                .send()
                .on("transactionHash", (hash) => {
                  dispatch(buyingLevel());
                ...
                .then((res) => {
                  toast.success(t("global:level-success"), {
                    autoClose: 1000,
                  });
                  dispatch(getUserData(wallet));
                })
      ...
      toast.error(t("global:viewer-mode"));
    }
  };
  ...
  return (
    <div className="levels">
      <div className="levels__info">
        <Link to="/dashboard"> ID {user?.id} </Link> &#62;{" "}
        ...
        <Container fluid>
          <Row>
            {levels?.map((level, index) => {
              return (
                <Col xs={6} sm={6} lg={4} xl={3} xxl={3} key={index}>
                  <div
                  ...
                  >
                    ...
                    <div
                      className="stage__top"
                      onClick={() =>
                        navigate(`${window.location.pathname}/${index + 1}`)
                      }
                    >
                      <p>
